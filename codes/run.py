...
def main(args, config):
    ...
    if config['generate_cache']:
        ...
        config['bed_file'], config['bed_chr'] = validate_bed(args['bed_file']) if 'bed_file' in args.keys() else None
        if config['bed_file']: 
    ...
    if cache_folder:
        ...
        config['cache_folder'], config['cache_chr'] = validate_cache(cache_folder, pon_dict)
    ...
    tumor_bam = validate_bam(args['tumor_bam'], config)
    ...
    if is_anno:
        ...
        anno_df, original_columns, config['anno_chr'] = read_anno_csv(mut_file, config)
